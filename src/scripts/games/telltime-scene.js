  ...
  window.ClassmatesTelltimeScene = {
    onCorrect: function(idx) {
      ...
      FXCore.emit(s.w * 0.5, s.h * 0.35, count, {
        spread: 6, rise: 2, decay: 0.016, size: 3,
        color: 'rgba(220,190,120,0.8)', shape: 'star', endColor: 'rgba(180,150,80,0)'
      });
      FXCore.emit(s.w * 0.5, s.h * 0.4, Math.floor(count * 0.6), {
        spread: 2, rise: 0.8, decay: 0.03, size: 1.2,
    ...
    onComplete: function() {
      ...
      var brassColors = ['rgba(200,170,100,0.7)','rgba(220,190,120,0.7)','rgba(180,150,80,0.7)','rgba(255,220,150,0.7)','rgba(240,200,100,0.7)','rgba(190,160,90,0.7)','rgba(210,180,110,0.7)','rgba(230,200,130,0.7)'];
      for (var j = 0; j < brassColors.length; j++) {
        FXCore.emit(s.w * (0.08 + j * 0.12), s.h * (0.25 + Math.sin(j) * 0.1), 8, {
          spread: 8, rise: 3, decay: 0.01, size: 4, color: brassColors[j], shape: 'star'
        });
      }
